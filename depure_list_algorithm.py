...
file_location = raw_input("write the location of the csv file: ")
if file_location == "0":
...
#save the value
f = csv.writer(open(file_location.replace(".csv", "") + "_depure.csv", "wb+"))
...
for x in entries:
    f.writerow([x["date"], x["user"], x["full_name"], x["tweet"].encode('utf-8'), x["id"], x["app"], x["followers"], x["follows"], x["retweets"], x["favorites"], x["verified"], x["user_since"], x["location"], x["bio"], x["profile_img"], x["maps"], x["depured"].encode('utf-8')])
